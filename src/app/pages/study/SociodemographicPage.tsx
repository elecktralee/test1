...
interface FieldProps { label: string; required?: boolean; children: React.ReactNode; error?: boolean }
...
export default function SociodemographicPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      navigate("/bai"); // BAI vem antes do CSS-33
    } catch {
    ...
    <StudyLayout currentStep="socio">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100">
        ...
        <div className="p-6 space-y-5">
          ...
          <Field label="Com qual gênero você se identifica?" required error={!!errors.gender}>
          ...
          <Field label="Nível de escolaridade" required error={!!errors.education}>
          ...
          <Field label="Estado civil" required error={!!errors.maritalStatus}>
          ...
          <Field label="Renda mensal familiar aproximada" required error={!!errors.monthlyIncome}>
          ...
          <Field label="Quantas horas por dia, em média, você usa a internet?" required error={!!errors.internetHours}>
          ...
          <Field label="Você possui alguma condição de saúde crônica diagnosticada?" required error={!!errors.chronicCondition}>
            <Sel field="chronicCondition" opts={[["Sim","Sim"],["Não","Não"],["Prefiro não informar","Prefiro não informar"]]} />
          </Field>

          <Field label="Você já recebeu diagnóstico de transtorno de ansiedade?" required error={!!errors.psychiatricDiagnosis}>
            <Sel field="psychiatricDiagnosis" opts={[["Sim","Sim"],["Não","Não"],["Prefiro não responder","Prefiro não responder"]]} />
          </Field>

          <Field label="Você faz uso regular de algum medicamento?" required error={!!errors.medications}>
            <Sel field="medications" opts={[["Sim","Sim"],["Não","Não"],["Prefiro não informar","Prefiro não informar"]]} />
          </Field>

          <Field label="Com que frequência você busca informações de saúde na internet?" required error={!!errors.healthSearchFrequency}>
          ...
          <Field label="Qual é o seu principal tipo de acesso a serviços de saúde?" required error={!!errors.healthcareAccess}>
